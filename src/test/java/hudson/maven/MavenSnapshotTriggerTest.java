 ...
 */
@WithJenkins
class MavenSnapshotTriggerTest {
     ...
     */
    @Test
    void testSnapshotDependencyBuildTrigger() throws Exception {
        Maven36xBuildTest.configureMaven36();
        MavenModuleSet projA = j.createProject(MavenModuleSet.class, "snap-dep-test-up");
        ...
        MavenModuleSet projB = j.createProject(MavenModuleSet.class, "snap-dep-test-down");
        ...
        j.buildAndAssertSuccess(projA);
        j.buildAndAssertSuccess(projB);
        ...
        j.waitUntilNoActivityUpTo(90*1000);
        assertEquals(2, projB.getLastBuild().getNumber(), "Expected most recent build of second project to be #2");
    }
     ...
     */
    @Test
    void testMixedTransitiveSnapshotTrigger() throws Exception {
        Maven36xBuildTest.configureMaven36();
        ...
        MavenModuleSet projB = j.createProject(MavenModuleSet.class, "snap-dep-test-mid");
        projB.setGoals("clean install -Dmaven.compiler.target=1.8 -Dmaven.compiler.source=1.8");
        projB.setIgnoreUpstremChanges(false);
        projB.setQuietPeriod(0);
        projB.setScm(new ExtractResourceSCM(getClass().getResource("maven-dep-test-B.zip")));

        MavenModuleSet projC = j.createProject(MavenModuleSet.class, "snap-dep-test-down");
        ...
        j.buildAndAssertSuccess(projA);
        j.buildAndAssertSuccess(projB);
        j.buildAndAssertSuccess(projC);

        projA.setScm(new ExtractResourceSCM(getClass().getResource("maven-dep-test-A-changed.zip")));

        j.buildAndAssertSuccess(projA);

        j.waitUntilNoActivityUpTo(90*1000);  // wait until dependency build trickles down
        assertEquals(2, projB.getLastBuild().getNumber(), "Expected most recent build of second project to be #2");
        assertEquals(2, projC.getLastBuild().getNumber(), "Expected most recent build of third project to be #2");
    }
     ...
     */
    @Test
    void testMultipleDependencySnapshotTrigger() throws Exception {
        Maven36xBuildTest.configureMaven36();

        // This is only executed to make sure that dependency A is available in repository
        MavenModuleSet prepareProject = j.createProject(MavenModuleSet.class, "prepareProject");
        ...
        MavenModuleSet projA2 = j.createProject(MavenModuleSet.class, "snap-dep-test-A2");
        ...
        MavenModuleSet projB1 = j.createProject(MavenModuleSet.class, "snap-dep-test-B1");
        ...
        MavenModuleSet projB2 = j.createProject(MavenModuleSet.class, "snap-dep-test-B2");
        ...
        MavenModuleSet projC = j.createProject(MavenModuleSet.class, "snap-dep-test-C");
        ...
        j.buildAndAssertSuccess(projA1);
        j.buildAndAssertSuccess(projA2);
        j.buildAndAssertSuccess(projB1);
        j.buildAndAssertSuccess(projB2);
        j.buildAndAssertSuccess(projC);
        ...
        assertEquals(2, upstreamProjectNames.size(), "Expected number of upstream project for project 'snap-dep-test-down' to be #2");
     ...
     */
    @Test
    void testMultipleDependencySnapshotTriggerIgnoreUnsuccessfullUpstreams() throws Exception {
        Maven36xBuildTest.configureMaven36();
        ...
        assertEquals(1, projC.getLastBuild().getNumber(), "Expected legacy behaviour, most recent build of project C to be #1");
        ...
        assertEquals(2, projC.getLastBuild().getNumber(), "Expected most recent build of project C to be #2");
    }
}
